...
| [`P3-CAND-02`](public_cases/P3-CAND-02-negative-coin-balance.md)<br>Negative coin balance / settle-drop<br>[Calculation](https://github.com/gonkavip/unclaimed); [independent audit](public_cases/sources/P3-CAND-02-independent-audit.md) | 1-274; tail 275-280 checked | Calculated; true archive validation required | Evgenii Maksimenkov | 19 miners; Evgenii Maksimenkov | @maksimenkoff; deterministic affected set calculated in [gonkavip/unclaimed](https://github.com/gonkavip/unclaimed); independent first-pass/code audit completed | @dem_ww; required: archive `SettleAmount` validation and decision | 1,075.336 GNK | Calculated but no `SettleAmount` was written; rewards were eventually swept into the gov module account. Independent summary scan found 823 broad positive-unclaimed rows in 1-274, proving archive state is required to isolate the 19 bug rows; authenticated `rpc.gonka.gg` still lacks the old state checked. | [#550](https://github.com/gonka-ai/gonka/pull/550) (`v0.2.8`) present in current code; current tail 275-280 has no positive-unclaimed rows; [#826](https://github.com/gonka-ai/gonka/pull/826) is a separate claim-time path |
