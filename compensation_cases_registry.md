...
| `P3-CAND-04` | UpgradeProtectionWindow / CPoC misfire | Независимо провалидирован, root cause подтверждён, требуется решение | 276 | Votkon; расчёт от Evgenii Maksimenkov | 19 miners; Evgenii Maksimenkov | @maksimenkoff; calculation published, Nik prior script result confirmed; GRC independent archive replay matched published CSV exactly: `7` dropped, `12` reduced; historical `LastUpgradeHeight` key `0x1b` is `null` at upgrade/cPoC heights; scan `270..283` finds only `2` case4-like rows, both inside epoch `276`; code fix identified in [PR #1268](https://github.com/gonka-ai/gonka/pull/1268) on `upgrade-v0.2.14` | @votkon; @OpenMindedPerson; inclusion decision and duplicate-overlap handling; confirm `v0.2.14` deployment separately | [gonkavip/payout276](https://github.com/gonkavip/payout276); [validation](validations/P3-CAND-04-upgrade-protection-cpoc-misfire-epoch-276/README.md) | 36,209.451291351 GNK | Решить включение, проверить overlap и подтвердить deployment fix перед утверждением recurrence risk | [досье](cases/P3-CAND-04-upgrade-protection-cpoc-misfire-epoch-276.md) |
...
|---|---|---|---|---|---|---|---|---:|---|---|
| `P3-CAND-05` | `ml3` hardware re-registration | Проверено независимо; требуется решение о scope; не входит в текущие пять назначенных кейсов Proposal #3 | `263..283`, фокус около `269` | SegovChik от `@gonkstein` и его technical contact | `gonka15munk...`; `@gonkstein` через SegovChik | Chain trace подтверждает node/weight transitions вокруг `ml3`; on-chain HW proof, `POC_SLOT=true` preservation по `ml3` и compensable loss не доказаны; PR #1089 является общей preserved-node mitigation | Только если committee принимает hardware replacement claims в scope | Не рассчитано | Policy/scope decision; off-chain proof и formula required | [досье](cases/P3-CAND-05-ml3-hardware-reregistration.md); [validation](validations/P3-CAND-05-ml3-hardware-reregistration/) |
| `P3-CAND-06` | Pre-fix confirmation accounting / pass-weight but failed ratio | Root-cause, raw stage, coefficient/new-algorithm replay, code-diff proof, one-page summary, row-by-row evidence ledger, gross compensation calculation, epoch/upgrade timestamp table, and post-upgrade regression scan added; требуется eligibility и overlap review | 263-276; post-upgrade checked 277-283 | Выявлено при validation `P3-CAND-03` | 19 unique participants / 24 epoch rows; contacts не сопоставлены | @mikenosov: archive scan `262..276`; GRC replay: 48/48 raw stage rows match aggregates, 25 pass-weight model rows cover all 24 candidates; old formula matches `22/24`; bounded new-style replay has `0/24` pass alpha; `v0.2.13` applied in epoch `276`, epoch `277` first clean start; post-upgrade scan `277..283` finds `8` broad single-model-pass hits and `0` both-model-pass hits | Да: eligibility for 20 non-epoch-276 formula-reconciled single-model rows, overlap checks with `P3-CAND-04` and `P4-CAND-01`; decide whether post-upgrade single-model rows are expected multi-model accounting | 120,822.324371792 GONKA gross before overlap | Решить eligibility для single-model rows и проверить overlap | [досье](cases/P3-CAND-06-pre-fix-confirmation-accounting.md) |
...
| `P3-CAND-04` | Independent validation готова и совпала с published CSV; root cause подтверждён historical `LastUpgradeHeight = null` и scope scan `270..283`; code fix найден в PR #1268 / `upgrade-v0.2.14`; нужны inclusion decision, duplicate-overlap handling и deployment confirmation. |
