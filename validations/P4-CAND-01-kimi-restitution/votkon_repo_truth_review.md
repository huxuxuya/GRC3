...
| Epoch `266` has nonce exclusion. | `Technically plausible, policy contested` | The script reconstructs weights from PoC commits and uses historical scale factors: Kimi `1.2620856201975851`, Qwen `0.3593`. This is a reconstruction, not a direct chain settlement replay. |
| Delegators in e266 lost extra weight because their operator was excluded. | `Plausible, needs policy` | The `10%` delta between `ModeNone` and `ModeDelegate` is coherent, but eligibility depends on accepting attack-induced operator exclusion as compensable. |
