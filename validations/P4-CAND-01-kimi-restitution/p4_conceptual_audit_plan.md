...
| 4 | The other two epoch `265` rows are direct cPoC restitution victims. | Per-address chain trace showing the same direct cPoC failure signature, not only confirmation-weight drop. | Audit pass 02 shows `gonka1830...` is a real zero-reward `failed_confirmation_poc` candidate that still needs row-level cPoC/model evidence; `gonka17...` is a rewarded confirmation-weight-drop row, not a direct zero-reward cPoC victim in the current evidence. | `Partially confirmed` | Verify cPoC submissions/validator path for `gonka1830...`; require policy decision for rewarded cW-drop rows like `gonka17...`. |
