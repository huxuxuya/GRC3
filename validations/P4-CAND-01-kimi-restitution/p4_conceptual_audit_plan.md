...
| e266 zero-reward rows | [`p4_problem_02a_e266_zero_reward_rows.md`](p4_problem_02a_e266_zero_reward_rows.md) | Formal remark `P4-E266-ZR-01` for 5 in-final-group failed-confirmation rows. |
| e266 rewarded top-up rows | [`p4_problem_02b_e266_rewarded_topup_rows.md`](p4_problem_02b_e266_rewarded_topup_rows.md) | Formal remark `P4-E266-TOPUP-01` for 4 already-rewarded reconstruction rows. |
...
| 8 | Epoch `266` reconstructed nonce weights should be used as the reward basis. | On-chain commit counts, historical model scale factors, and an accepted reconstruction rule. | Pass 04 shows the source nonce table has `18` rows: `9` excluded operators, `4` in-final-group rewarded top-up rows, and `5` in-final-group zero-reward rows. Pass 02b marks the 4 rewarded rows as `P4-E266-TOPUP-01`: already-rewarded reconstruction/top-up policy rows. | `Policy required` | Decide whether reconstructed PoC work can compensate top-up rows that were in the final epoch group. |
| 9 | Epoch `266` zero-reward submitters are victims, not ordinary failed participants. | Causal proof that exclusion came from the claimed incident, not local failure, invalid work, or protocol-valid exclusion. | Pass 02a confirms all `5` rows were in final group, had raw commits and validation records, then ended with `confirmation_weight=0`, `rewarded_coins=0`, and `failed_confirmation_poc`. They are not the same class as the 9 absent operators; three rows are Qwen-only by source labels. | `Partially confirmed` | Keep these rows separate and require row-level cause/policy before compensation. |
...
| 13 | `ComputeGroupCap` was a protocol bug rather than intended balancing behavior. | Code intent, parameter history, reviewer statements, and mitigation rationale. | Public case notes that `ComputeGroupCap` is an intended rule and that GRC reviewers questioned compensating it. Marked as `P4-GC-01`. | `Policy required` | Committee must decide whether intended cap behavior can become compensable due to context. |
...
| 15 | The source e267-e276 formula is chain-correct. | Full reward replay or protocol-level proof that capped `root_total_weight` should remain the denominator. | Pass 06 shows material denominator sensitivity across epochs `267..276`: source capped-root top-up totals `727,219.351170981` GONKA, all-root-confirmation denominator totals `503,653.983658046` GONKA, and replace-affected denominator totals `360,858.547914700` GONKA. Marked as `P4-GC-02`. | `Policy required` | Choose one model: top-up against paid settlement, all-confirmation denominator, or another full replay rule. |
